 ...
 * entity of type object really is.  Two obvious choices are a pointer (to
 * e actual object memory) or an index into an object table.  We decided to
 * se the latter, although either would work.
 * 
 * Similarly, one can either define the token object using a typedef, or using
 * a define statement.  Either one will work (check this?)
 */ 
 ...
 * one.  However, for efficiency reasons, many of these procedures can be
 * laced by macros generating in-line code.  For the latter approach to
 * work, the structure of the object table must be known.  For this reason,
 * t is given here.  Note, however, that outside of the files memory.c and
 * ixio.c (or macio.c on the macintosh) ONLY the macros described in this
 * e make use of this structure: therefore modifications or even complete
 * lacement is possible as long as the interface remains consistent
 */ 
 ...
 * increment and decrement reference counts in objects.  By separating
 * ent from memory freeing, we could replace these as procedure calls by
 * using the following macros (thereby saving procedure calls):
 */ 
 ...
 * is in case evaluation of x results in side effects (such as assignment)
 ...
 * llocStr(str) - allocate a string and fill it in
 * 
 ...
 * integers are changed to x*2+1.  Either a negative or an odd number is
 * therefore an integer, while a nonzero even number is an object pointer
 * (multiplied by two).  Zero is reserved for the object ``nil'' Since
 * newInteger does not fill in the class field, it can be given here. If it
 * was required to use the class field, it would have to be deferred until
 * names.h
 */ 
 ...
 * i) - ith field (start at 1) of object x basicAtPut(x, i, v) - put value v
 ...
 * fields and size fields of objects
 */ 
 ...
 * ures in the Unix world.  Some perform very badly when asked to allocate
 * thousands of very small memory blocks, while others take this without any
 * difficulty.  The routine mBlockAlloc is used to allocate a small bit of
 * memory; the version given below allocates a large block and then chops it
 * up as needed; if desired, for versions of malloc that can handle small
