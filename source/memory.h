 ...
 * entity of type object really is.  Two obvious choices are a pointer
 * (to the actual object memory) or an index into an object table.  We
 * decided to use the latter, although either would work.
 *
 ...
 * which increment and decrement reference counts in objects. By
 * separating ent from memory freeing, we could replace these as
 * procedure calls by using the following macros (thereby saving
 * procedure calls):
 */
 ...
 * object pointer (multiplied by two).  Zero is reserved for the object
 * ``nil''
 ...
 * ures in the Unix world.  Some perform very badly when asked to allocate
