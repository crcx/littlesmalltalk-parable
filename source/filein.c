 ...
 * Routines used in reading in textual descriptions of classes.
  ************************************************************************/ 
/*
 *  Little Smalltalk, version 3 Written by Tim Budd, Oregon State
 ...
 * fer.  In addition, all methods must also fit into this buffer.
 */ 
...
/*
 *  findClass gets a class object, either by finding it already or making
...
fileIn(FILE * fd, boolean printit) 
{
  ...
  while (fgets(textBuffer, TextBufferSize, fd) != NULL)
  {
    ...
    lexinit(textBuffer);
      ...
      else if ((token == binary) && streq(tokenString, "*"));	/* do nothing, its a
								 * comment */
